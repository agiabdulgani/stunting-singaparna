        ...
        <div class="row g-3">
            ...
            <div class="col-6 col-md-4 col-xl-2">
                <div class="card card-stat p-3 text-center">
                    ...
                    <div class="stat-value fs-3 fw-bold text-primary mt-1">
                        {{ number_format($populationTotal, 0, ',', '.') }}
                    </div>
                    <small class="text-muted">KK: {{ number_format($familyTotal, 0, ',', '.') }}</small>
                </div>
            </div>
            ...
            <div class="col-6 col-md-4 col-xl-2">
                <div class="card card-stat p-3 text-center">
                    ...
                    <div class="stat-value fs-3 fw-bold text-success mt-1">
                        {{ number_format($educationTotal, 0, ',', '.') }}
                    </div>
                    <small class="text-muted">
                        PAUD {{ $paudTotal }} · SMP {{ $smpTotal }} · SMA {{ $smaTotal }}
                    </small>
                </div>
            </div>
            ...
            <div class="col-6 col-md-4 col-xl-2">
                <div class="card card-stat p-3 text-center">
                    ...
                    <div class="stat-value fs-3 fw-bold text-warning mt-1">
                        {{ number_format($constraintTotal, 0, ',', '.') }}
                    </div>
            ...
            <div class="col-6 col-md-4 col-xl-2">
                <div class="card card-stat p-3 text-center">
                    ...
                    <small class="text-muted">{{ $budgetRows->count() }} data</small>
                </div>
            </div>
            ...
            <div class="col-6 col-md-4 col-xl-2">
                <div class="card card-stat p-3 text-center">
                    ...
                    <small class="text-muted">KIA {{ $kiaTotal }} · Pangan {{ $foodTotal }}</small>
                </div>
            </div>
            ...
            <div class="col-6 col-md-4 col-xl-2">
                <div class="card card-stat p-3 text-center">
                    ...
                    <div class="stat-value fs-3 fw-bold text-danger mt-1">
                        {{ number_format($mbgPortionTotal, 0, ',', '.') }}
                    </div>
            ...
            <div class="card card-stat p-3">
                <div class="d-flex justify-content-between">
                    <div>
                        ...
                        <div class="fs-4 fw-bold">{{ number_format($youngFemaleTotal, 0, ',', '.') }}</div>
                    </div>
            ...
            <div class="card card-stat p-3">
                <div class="d-flex justify-content-between">
                    <div>
                        ...
                        <div class="fs-4 fw-bold">{{ number_format($childrenTotal, 0, ',', '.') }}</div>
                    </div>
            ...
            <div class="card card-stat p-3">
                <div class="d-flex justify-content-between">
                    <div>
                        ...
                        <div class="fs-4 fw-bold">{{ number_format($guruPaudTotal, 0, ',', '.') }}</div>
                    </div>
            ...
            <div class="card card-stat p-3">
                <div class="d-flex justify-content-between">
                    <div>
                        ...
                        <div class="fs-4 fw-bold">{{ number_format($individualTotal, 0, ',', '.') }}</div>
                    </div>
    ...
    <div class="tab-content" id="pills-tabContent">
            ...
            <div class="card content-card p-4 mb-4">
                <div class="d-flex flex-wrap justify-content-between gap-2 mb-3">
                    ...
                    <span class="badge bg-primary-subtle text-primary badge-soft">
                        {{ $targetData instanceof \Illuminate\Contracts\Pagination\Paginator || $targetData instanceof \Illuminate\Contracts\Pagination\LengthAwarePaginator ? $targetData->total() : $targetRows->count() }}
                        data
                    </span>
                </div>

                <form action="{{ route('target.store') }}" method="POST" class="row g-3 mb-4 no-print">
                    @csrf

                    <div class="col-12 col-lg-3">
                        <label class="form-label small fw-bold">Nama Desa / Kelurahan</label>
                        <input type="text" name="village_name" class="form-control"
                               placeholder="Contoh: Singaparna" value="{{ old('village_name') }}" required>
                    </div>
                    ...
                    <table class="table table-striped table-hover align-middle exportable-table" id="table-sasaran">
                        ...
                        <tbody>
                            @forelse($targetRows as $target)
                                <tr>
                                    <td class="fw-bold">{{ $target->village_name ?? '-' }}</td>
                                    <td class="text-end number-cell">{{ number_format((int)($target->population_count ?? 0), 0, ',', '.') }}</td>
                                    <td class="text-end number-cell">{{ number_format((int)($target->family_count ?? 0), 0, ',', '.') }}</td>
                                    <td class="text-end number-cell">{{ number_format((int)($target->young_female_count ?? 0), 0, ',', '.') }}</td>
                                    <td class="text-end number-cell">{{ number_format((int)($target->children_count ?? 0), 0, ',', '.') }}</td>
                                    <td class="text-end no-print">
                                        @if(auth()->user() && auth()->user()->role === 'admin')
                                        <form action="{{ route('target.destroy', $target->id) }}" method="POST" class="d-inline delete-form">
                                            @csrf
                                            @method('DELETE')
                        ...
                        <tfoot class="table-light fw-bold">
                            <tr>
                                <td>TOTAL</td>
                                <td class="text-end">{{ number_format($populationTotal, 0, ',', '.') }}</td>
                                <td class="text-end">{{ number_format($familyTotal, 0, ',', '.') }}</td>
                                <td class="text-end">{{ number_format($youngFemaleTotal, 0, ',', '.') }}</td>
                                <td class="text-end">{{ number_format($childrenTotal, 0, ',', '.') }}</td>
                ...
                @if(is_object($targetData) && method_exists($targetData, 'links'))
                    {{ $targetData->appends(['mbg_page' => is_object($mbgData) && method_exists($mbgData, 'currentPage') ? $mbgData->currentPage() : 1])->links() }}
                @endif
            </div>
        </div>
            ...
            <div class="card content-card p-4 mb-4">
                <div class="d-flex flex-wrap justify-content-between gap-2 mb-3">
                    ...
                    <div class="d-flex gap-2 flex-wrap">
                        <span class="badge bg-success-subtle text-success badge-soft">PAUD: {{ $paudTotal }}</span>
                        <span class="badge bg-primary-subtle text-primary badge-soft">SMP: {{ $smpTotal }}</span>
                        <span class="badge bg-warning-subtle text-warning badge-soft">SMA: {{ $smaTotal }}</span>
                    </div>
                </div>

                <!-- FIX: sebelumnya ada 3 <form> bersarang (nested) untuk endpoint yang sama.
                     Nested <form> tidak valid dalam HTML dan bisa membuat @csrf/token gagal
                     terkirim dengan benar. Sekarang hanya SATU form dengan SATU @csrf. -->
                <form action="{{ route('support.store') }}" method="POST" class="row g-3 mb-4 no-print">
                    @csrf
                ...
                <div class="row g-3 mb-3">
                    <div class="col-6 col-md-3">
                        <div class="border rounded-3 p-3 bg-light">
                            ...
                            <div class="fs-4 fw-bold text-success">{{ number_format($paudTotal, 0, ',', '.') }}</div>
                        </div>
                    </div>
                    <div class="col-6 col-md-3">
                        <div class="border rounded-3 p-3 bg-light">
                            <div class="small text-muted">SMP Sederajat</div>
                            <div class="fs-4 fw-bold text-primary">{{ number_format($smpTotal, 0, ',', '.') }}</div>
                        </div>
                    </div>
                    <div class="col-6 col-md-3">
                        <div class="border rounded-3 p-3 bg-light">
                            <div class="small text-muted">SMA Sederajat</div>
                            <div class="fs-4 fw-bold text-warning">{{ number_format($smaTotal, 0, ',', '.') }}</div>
                        </div>
                    </div>
                    <div class="col-6 col-md-3">
                        <div class="border rounded-3 p-3 bg-light">
                            <div class="small text-muted">Guru PAUD</div>
                            <div class="fs-4 fw-bold text-info">{{ number_format($guruPaudTotal, 0, ',', '.') }}</div>
                        </div>
                    </div>
                </div>
                ...
                <div class="table-responsive">
                    <table class="table table-striped table-hover align-middle exportable-table" id="table-pendukung">
                        ...
                        <tbody>
                            @forelse($supportRows as $support)
                                <tr>
                                    <td class="fw-bold">{{ $support->village_name ?? '-' }}</td>
                                    <td class="text-end number-cell">{{ number_format((int)($support->jml_paud ?? 0), 0, ',', '.') }}</td>
                                    <td class="text-end number-cell">{{ number_format((int)($support->smp_mts ?? 0), 0, ',', '.') }}</td>
                                    <td class="text-end number-cell">{{ number_format((int)($support->sma_ma ?? 0), 0, ',', '.') }}</td>
                                    <td class="text-end number-cell">{{ number_format((int)($support->guru_paud ?? 0), 0, ',', '.') }}</td>
                                    <td class="text-end no-print">
                                        @if(auth()->user() && auth()->user()->role === 'admin')
                                        <form action="{{ route('support.destroy', $support->id) }}" method="POST" class="d-inline delete-form">
                                            @csrf
                                            @method('DELETE')
                                            <button type="submit" class="btn btn-sm btn-danger" title="Hapus data">
                                                <i class="fa-solid fa-trash"></i>
                        ...
                        <tfoot class="table-light fw-bold">
                            <tr>
                                <td>TOTAL</td>
                                <td class="text-end">{{ number_format($paudTotal, 0, ',', '.') }}</td>
                                <td class="text-end">{{ number_format($smpTotal, 0, ',', '.') }}</td>
                                <td class="text-end">{{ number_format($smaTotal, 0, ',', '.') }}</td>
                                <td class="text-end">{{ number_format($guruPaudTotal, 0, ',', '.') }}</td>
                                <td class="no-print"></td>
                            </tr>
                        </tfoot>
                    </table>
            ...
            <div class="card content-card p-4 mb-4">
                <div class="d-flex flex-wrap justify-content-between gap-2 mb-3">
                    ...
                    <span class="badge bg-warning-subtle text-warning badge-soft">
                        {{ number_format($constraintTotal, 0, ',', '.') }} kendala
                    </span>
                </div>

                <form action="{{ route('constraint.store') }}" method="POST" class="row g-3 mb-4 no-print">
                    @csrf

                    <div class="col-12 col-md-4">
                        <label class="form-label small fw-bold">Desa / Kelurahan</label>
                        <input type="text" name="village_name" class="form-control"
                               value="{{ old('village_name') }}" required>
                    </div>
                ...
                <div class="table-responsive">
                    <table class="table table-striped table-hover align-middle exportable-table" id="table-kendala">
                        ...
                        <tbody>
                            @forelse($constraintRows as $item)
                                <tr>
                                    <td class="fw-bold">{{ $item->village_name ?? '-' }}</td>
                                    <td>{{ $item->description ?? '-' }}</td>
                                    <td class="text-end no-print">
                                        @if(auth()->user() && auth()->user()->role === 'admin')
                                        <form action="{{ route('constraint.destroy', $item->id) }}" method="POST" class="d-inline delete-form">
                                            @csrf
                                            @method('DELETE')
                                            <button type="submit" class="btn btn-sm btn-danger" title="Hapus data">
                                                <i class="fa-solid fa-trash"></i>
                        ...
                        <tfoot class="table-light fw-bold">
                            <tr>
                                <td>TOTAL KENDALA</td>
                                <td>{{ number_format($constraintTotal, 0, ',', '.') }} data</td>
                                <td class="no-print"></td>
                            </tr>
                        </tfoot>
                    </table>
        ...
        <div class="tab-pane fade" id="content-anggaran" role="tabpanel">
            <div class="card content-card p-4 mb-4">
                ...
                <form action="{{ route('budget.store') }}" method="POST" class="row g-3 mb-4 no-print">
                    @csrf
                    ...
                    <div class="col-12 col-md-3">
                        ...
                        <input type="text" name="indicator_name" class="form-control"
                               value="{{ old('indicator_name') }}" required>
                    </div>
                    ...
                    <div class="col-12 col-md-3">
                        ...
                        <input type="text" name="activity_description" class="form-control"
                               value="{{ old('activity_description') }}" required>
                    </div>
                ...
                <div class="table-responsive">
                    <table class="table table-striped table-hover align-middle exportable-table" id="table-anggaran">
                        ...
                        <tbody>
                            @forelse($budgetRows as $budget)
                                <tr>
                                    <td class="fw-bold">{{ $budget->village_name ?? '-' }}</td>
                                    <td>{{ $budget->indicator_name ?? '-' }}</td>
                                    <td class="text-end number-cell">
                                        Rp {{ number_format((float)($budget->amount ?? 0), 0, ',', '.') }}
                                    </td>
                                    <td>{{ $budget->activity_description ?? '-' }}</td>
                                    <td class="text-end no-print">
                                        @if(auth()->user() && auth()->user()->role === 'admin')
                                        <form action="{{ route('budget.destroy', $budget->id) }}" method="POST" class="d-inline delete-form">
                                            @csrf
                                            @method('DELETE')
                                            <button type="submit" class="btn btn-sm btn-danger" title="Hapus data">
                                                <i class="fa-solid fa-trash"></i>
                        ...
                        <tfoot class="table-light fw-bold">
                            <tr>
                                ...
                                <td class="text-end">Rp {{ number_format($budgetTotal, 0, ',', '.') }}</td>
                                <td></td>
                                <td class="no-print"></td>
                            </tr>
                        </tfoot>
        ...
        <div class="tab-pane fade" id="content-layanan" role="tabpanel">
            <div class="card content-card p-4 mb-4">
                ...
                <form action="{{ route('service.store') }}" method="POST" class="row g-3 mb-4 no-print">
                    @csrf
                    ...
                    <div class="col-12 col-md-4">
                        ...
                        <input type="number" min="0" name="birth_kia_count" class="form-control"
                               value="{{ old('birth_kia_count', 0) }}" required>
                    </div>
                    ...
                    <div class="col-12 col-md-4">
                        ...
                        <input type="number" min="0" name="food_program_count" class="form-control"
                               value="{{ old('food_program_count', 0) }}" required>
                    </div>
                ...
                <div class="row g-3 mb-3">
                    <div class="col-6 col-md-4">
                        <div class="border rounded-3 p-3 bg-light">
                            ...
                            <div class="fs-4 fw-bold text-primary">{{ number_format($kiaTotal, 0, ',', '.') }}</div>
                        </div>
                    </div>
                    <div class="col-6 col-md-4">
                        <div class="border rounded-3 p-3 bg-light">
                            <div class="small text-muted">Program Pangan</div>
                            <div class="fs-4 fw-bold text-success">{{ number_format($foodTotal, 0, ',', '.') }}</div>
                        </div>
                    </div>
                    <div class="col-12 col-md-4">
                        <div class="border rounded-3 p-3 bg-light">
                            <div class="small text-muted">Total Capaian</div>
                            <div class="fs-4 fw-bold text-secondary">{{ number_format($serviceTotal, 0, ',', '.') }}</div>
                        </div>
                    </div>
                </div>
                    ...
                    <table class="table table-striped table-hover align-middle exportable-table" id="table-layanan">
                        ...
                        <tbody>
                            @forelse($serviceRows as $service)
                                <tr>
                                    <td class="fw-bold">{{ $service->village_name ?? '-' }}</td>
                                    <td class="text-end number-cell">{{ number_format((int)($service->birth_kia_count ?? 0), 0, ',', '.') }}</td>
                                    <td class="text-end number-cell">{{ number_format((int)($service->food_program_count ?? 0), 0, ',', '.') }}</td>
                                    <td class="text-end no-print">
                                        @if(auth()->user() && auth()->user()->role === 'admin')
                                        <form action="{{ route('service.destroy', $service->id) }}" method="POST" class="d-inline delete-form">
                                            @csrf
                                            @method('DELETE')
                                            <button type="submit" class="btn btn-sm btn-danger" title="Hapus data">
                                                <i class="fa-solid fa-trash"></i>
                        ...
                        <tfoot class="table-light fw-bold">
                            <tr>
                                <td>TOTAL</td>
                                <td class="text-end">{{ number_format($kiaTotal, 0, ',', '.') }}</td>
                                <td class="text-end">{{ number_format($foodTotal, 0, ',', '.') }}</td>
                                <td class="no-print"></td>
                            </tr>
                        </tfoot>
                    </table>
            ...
            <div class="card content-card p-4 mb-4">
                <div class="d-flex flex-wrap justify-content-between gap-2 mb-3">
                    ...
                    <span class="badge bg-danger-subtle text-danger badge-soft">
                        Porsi: {{ number_format($mbgPortionTotal, 0, ',', '.') }}
                    </span>
                </div>

                <form action="{{ route('mbg.store') }}" method="POST" class="row g-3 mb-4 no-print">
                    @csrf
                    ...
                    <div class="col-6 col-lg-3">
                        ...
                        <input type="number" min="0" name="porsi_terdistribusi_harian" class="form-control"
                               value="{{ old('porsi_terdistribusi_harian', 0) }}" required>
                    </div>
                ...
                <div class="row g-3 mb-3">
                    <div class="col-6 col-lg-3">
                        <div class="border rounded-3 p-3 bg-light">
                            ...
                            <div class="fs-4 fw-bold text-primary">{{ number_format($mbgBalitaTotal, 0, ',', '.') }}</div>
                        </div>
                    </div>
                    <div class="col-6 col-lg-3">
                        <div class="border rounded-3 p-3 bg-light">
                            <div class="small text-muted">Bumil</div>
                            <div class="fs-4 fw-bold text-danger">{{ number_format($mbgBumilTotal, 0, ',', '.') }}</div>
                        </div>
                    </div>
                    <div class="col-6 col-lg-3">
                        <div class="border rounded-3 p-3 bg-light">
                            <div class="small text-muted">PAUD</div>
                            <div class="fs-4 fw-bold text-success">{{ number_format($mbgPaudTotal, 0, ',', '.') }}</div>
                        </div>
                    </div>
                    <div class="col-6 col-lg-3">
                        <div class="border rounded-3 p-3 bg-light">
                            <div class="small text-muted">Porsi Harian</div>
                            <div class="fs-4 fw-bold text-warning">{{ number_format($mbgPortionTotal, 0, ',', '.') }}</div>
                        </div>
                    </div>
                </div>
                ...
                <div class="table-responsive">
                    <table class="table table-striped table-hover align-middle exportable-table" id="table-mbg">
                        ...
                        <tbody>
                            @forelse($mbgRows as $mbg)
                                <tr>
                                    <td class="fw-bold">{{ $mbg->nama_desa ?? '-' }}</td>
                                    <td class="text-end number-cell">{{ number_format((int)($mbg->penerima_balita ?? 0), 0, ',', '.') }}</td>
                                    <td class="text-end number-cell">{{ number_format((int)($mbg->penerima_bumil ?? 0), 0, ',', '.') }}</td>
                                    <td class="text-end number-cell">{{ number_format((int)($mbg->penerima_paud ?? 0), 0, ',', '.') }}</td>
                                    <td class="text-end number-cell">{{ number_format((int)($mbg->porsi_terdistribusi_harian ?? 0), 0, ',', '.') }}</td>
                                    <td class="text-end no-print">
                                        @if(auth()->user() && auth()->user()->role === 'admin')
                                        <form action="{{ route('mbg.destroy', $mbg->id) }}" method="POST" class="d-inline delete-form">
                                            @csrf
                                            @method('DELETE')
                                            <button type="submit" class="btn btn-sm btn-danger" title="Hapus data">
                                                <i class="fa-solid fa-trash"></i>
                        ...
                        <tfoot class="table-light fw-bold">
                            <tr>
                                <td>TOTAL</td>
                                <td class="text-end">{{ number_format($mbgBalitaTotal, 0, ',', '.') }}</td>
                                <td class="text-end">{{ number_format($mbgBumilTotal, 0, ',', '.') }}</td>
                                <td class="text-end">{{ number_format($mbgPaudTotal, 0, ',', '.') }}</td>
                                <td class="text-end">{{ number_format($mbgPortionTotal, 0, ',', '.') }}</td>
                                <td class="no-print"></td>
                            </tr>
                        </tfoot>
                    </table>
                ...
                @if(is_object($mbgData) && method_exists($mbgData, 'links'))
                    {{ $mbgData->appends(['target_page' => is_object($targetData) && method_exists($targetData, 'currentPage') ? $targetData->currentPage() : 1])->links() }}
                @endif
            </div>
        </div>
            ...
            <div class="card content-card p-4 mb-4">
                <div class="d-flex flex-wrap justify-content-between gap-2 mb-3">
                    ...
                    <span class="badge bg-dark-subtle text-dark badge-soft">
                        {{ number_format($individualTotal, 0, ',', '.') }} data
                    </span>
                </div>

                <form action="{{ route('individual.store') }}" method="POST" class="row g-3 mb-4 no-print">
                    @csrf
                    ...
                    <div class="col-6 col-md-2">
                        ...
                        <input type="number" min="0" name="umur_bulan" class="form-control"
                               value="{{ old('umur_bulan', 0) }}" required>
                    </div>
                    ...
                    <div class="col-6 col-md-2">
                        ...
                        <input type="number" min="0" step="0.1" name="tinggi_badan" class="form-control"
                               value="{{ old('tinggi_badan', 0) }}" required>
                    </div>
                    ...
                    <div class="col-12 col-md-2">
                        ...
                        <input type="number" min="0" step="0.1" name="berat_badan" class="form-control"
                               value="{{ old('berat_badan', 0) }}" required>
                    </div>
                ...
                <div class="table-responsive">
                    <table class="table table-striped table-hover align-middle exportable-table" id="table-individu">
                        ...
                        <tbody>
                            @forelse($individualRows as $ind)
                                <tr>
                                    <td class="fw-bold">{{ $ind->nama_desa ?? '-' }}</td>
                                    <td>{{ $ind->nama_anak ?? '-' }}</td>
                                    <td class="text-end">{{ number_format((int)($ind->umur_bulan ?? 0), 0, ',', '.') }}</td>
                                    <td class="text-end">{{ number_format((float)($ind->tinggi_badan ?? 0), 1, ',', '.') }}</td>
                                    <td class="text-end">{{ number_format((float)($ind->berat_badan ?? 0), 1, ',', '.') }}</td>
                                    <td>{{ $ind->alamat ?? '-' }}</td>
                                    <td class="text-end no-print">
                                        @if(auth()->user() && auth()->user()->role === 'admin')
                                        <form action="{{ route('individual.destroy', $ind->id) }}" method="POST" class="d-inline delete-form">
                                            @csrf
                                            @method('DELETE')
                                            <button type="submit" class="btn btn-sm btn-danger" title="Hapus data">
                                                <i class="fa-solid fa-trash"></i>
                ...
                @if(is_object($individualData) && method_exists($individualData, 'links'))
                    {{ $individualData->links() }}
                @endif
            </div>
        </div>
